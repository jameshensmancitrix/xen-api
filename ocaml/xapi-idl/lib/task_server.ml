  ...
  struct
    ...
    type task_handle = {
      ...
      ; mutable cancel_points_seen: int
            (** incremented every time we pass a cancellation point *)
    ...
    let add tasks dbg (f : task_handle -> Interface.Task.async_result option) =
      ...
      let t =
        ...
        ; test_cancel_at=
            ( match tasks.test_cancel_trigger with
            | Some (dbg', n) when dbg = dbg' ->
                (* one shot *)
                clear_cancel_trigger tasks ; Some n
            | _ ->
                None
            )
    ...
    let to_interface_task t =
      ...
      ; state= t.state
      ; subtasks= t.subtasks
      ; debug_info= [("cancel_points_seen", string_of_int t.cancel_points_seen)]
    ...
    let set_state task state = task.state <- state

    let get_dbg task_handle = task_handle.dbg
