...
let register =
  call ~name:"register" ~in_oss_since:None ~lifecycle:[] ~hide_from_docs:true
    ~doc:"Register a new observer on all hosts"
    ~params:
      [
...
let unregister =
  call ~name:"unregister" ~in_oss_since:None ~lifecycle:[] ~hide_from_docs:true
    ~doc:"Unegister a new observer on all hosts"
    ~params:
      [
        (Ref _observer, "self", "The observer")
      ; (Ref _host, "host", "The host to be registered on")
...
let set_hosts =
  call ~name:"set_hosts" ~in_oss_since:None ~lifecycle:[]
    ...
    ~params:
      [
        (Ref _observer, "self", "The observer")
      ; (Set (Ref _host), "value", "Hosts the observer is to be registered on")
      ]
    ~allowed_roles:_R_POOL_ADMIN ()
...
let set_enabled =
  call ~name:"set_enabled" ~in_oss_since:None ~lifecycle:[]
    ~doc:
      "Enable / disable this observer which will stop the proivder from \
...
let set_attributes =
  call ~name:"set_attributes" ~in_oss_since:None ~lifecycle:[]
    ~doc:
      "Set the attributes field of a observer that are emitted to information \
       derived from this observer"
    ~params:
      [
        (Ref _observer, "self", "The observer")
      ; ( Map (String, String)
        , "value"
        , "The attributes that are emitted to data obtained from this observer"
        )
      ]
    ~allowed_roles:_R_POOL_ADMIN ()
...
let set_components =
  call ~name:"set_components" ~in_oss_since:None ~lifecycle:[]
    ~doc:
      "Set the components on which the observer is to broadcast to. i.e. xapi, \
       xenopds, networkd, etc"
    ~params:
      [
        (Ref _observer, "self", "The observer")
      ; ( Set String
        , "value"
        , "The components the observer is to be broadcast to"
        )
      ]
    ~allowed_roles:_R_POOL_ADMIN ()
...
let t =
  create_obj ~name:_observer
    ~descr:
      "Describes a observer which will control observability actvity in the \
       Toolstack"
    ...
    ~contents:
      ...
      @ [
          field ~qualifier:StaticRO ~ty:(Set (Ref _host)) ~lifecycle:[] "hosts"
            "A list of hosts the observer is to be registered on. If empty \
             this refers to all hosts"
            ~ignore_foreign_key:true ~default_value:(Some (VSet []))
        ; field ~qualifier:StaticRO
            ~ty:(Map (String, String))
            ~lifecycle:[] "attributes"
            "Attributes to be set on observers to provide extra information on \
             observers"
            ~default_value:(Some (VMap []))
        ; field ~qualifier:StaticRO ~ty:(Set String) ~lifecycle:[] "endpoints"
            "List of endpoints where data is exported to. Each endpoint is a \
             URL or the string 'bugtool' refering to our internal logs"
            ~default_value:(Some (VSet []))
        ; field ~qualifier:StaticRO ~ty:(Set String) ~lifecycle:[] "components"
            "List of componenets the observer is to be broadcast to. If empty \
             this refers to all components"
            ~default_value:(Some (VSet []))
