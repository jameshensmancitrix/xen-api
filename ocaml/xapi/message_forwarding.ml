  ...
  struct
    ...
    module VM = struct
      ...
      *)
      ...
      let forward_to_suitable_host ~local_fn ~__context ~vm ~snapshot ?host_op
          op =
        let suitable_host =
          Helpers.with_global_lock (fun () ->
              let host =
    ...
    module VIF = struct
      ...
      let forward_vif_op ~local_fn ~__context ~self op =
        ...
        if host_resident_on = Ref.null then
          ...
          do_op_on ~local_fn ~__context ~host:host_resident_on op
    ...
    module Observer = struct
      ...
      let set_enabled ~__context ~self ~value =
        ...
        |> List.iter (fun host -> do_op_on ~__context ~host ~local_fn client_fn) ;
        Db.Observer.set_enabled ~__context ~self ~value
      ...
      let set_attributes ~__context ~self ~value =
        ...
        |> Xapi_observer.observed_hosts_of ~__context
        |> List.iter (fun host -> do_op_on ~__context ~host ~local_fn client_fn) ;
        Db.Observer.set_attributes ~__context ~self ~value
      ...
      let set_endpoints ~__context ~self ~value =
        ...
        |> Xapi_observer.observed_hosts_of ~__context
        |> List.iter (fun host -> do_op_on ~__context ~host ~local_fn client_fn) ;
        Db.Observer.set_endpoints ~__context ~self ~value
