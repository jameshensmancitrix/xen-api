...
let assert_VGPU_type_supported ~__context ~self ~vgpu_type =
  ...
  in
  if not (List.mem vgpu_type supported_VGPU_types) then
    raise
...
let assert_VGPU_type_enabled ~__context ~self ~vgpu_type =
  ...
  let enabled_VGPU_types = Db.PGPU.get_enabled_VGPU_types ~__context ~self in
  if not (List.mem vgpu_type enabled_VGPU_types) then
    raise
      (Api_errors.Server_error
...
let get_remaining_capacity_internal ~__context ~self ~vgpu_type
  ...
  try
    ...
    then
      ...
      in
      let new_vgpu_size = Db.VGPU_type.get_size ~__context ~self:vgpu_type in
      convert_capacity
...
let assert_destination_pgpu_is_compatible_with_vm ~__context ~vm ~vgpu ~pgpu
    ~host ?remote () =
  ...
  let test_nvidia_compatibility vgpu pgpu =
    let pgpu_metadata =
      try get_compatibility_metadata pgpu |> List.assoc Xapi_gpumon.Nvidia.key
      with Not_found ->
        debug
          "Key %s is missing from the compatibility_metadata for pgpu %s on \
           the host %s."
...
let assert_destination_has_pgpu_compatible_with_vm ~__context ~vm ~vgpu_map
  ...
  let rec get_first_suitable_pgpu pgpu_types vgpu = function
    | [] ->
        debug "No compatible pGPU found on host %s, needed by VM %s"
          (Ref.string_of host) (Ref.string_of vm) ;
    ...
    | pgpu :: rest ->
        ...
        if Listext.List.set_equiv pgpu_types types then
          ...
          get_first_suitable_pgpu pgpu_types vgpu rest
  in
  let test_compatibility vgpu pgpus =
    ...
    | `passthrough | `gvt_g | `mxgpu ->
        ()
    | `nvidia | `nvidia_sriov ->
        Db.VGPU.get_GPU_group ~__context ~self:vgpu |> fun self ->
        Db.GPU_group.get_GPU_types ~__context ~self |> fun pgpu_types ->
        get_first_suitable_pgpu pgpu_types vgpu pgpus |> fun pgpu ->
        assert_destination_pgpu_is_compatible_with_vm ~__context ~vm ~vgpu ~pgpu
          ~host ?remote ()
  in
  let vgpus = Db.VM.get_VGPUs ~__context ~self:vm in
  let _mapped, unmapped =
    List.partition (fun vgpu -> List.mem_assoc vgpu vgpu_map) vgpus
  in
  ...
  List.iter
    (fun (vgpu, gpu_group) ->
      ...
      | `nvidia | `nvidia_sriov -> (
          ...
          | [] ->
              debug
                "No compatible pGPU found using the provided vgpu_map, needed \
                 by VM %s"
                (Ref.string_of vm) ;
              raise
                Api_errors.(
  ...
  List.iter (fun vgpu -> test_compatibility vgpu pgpus) unmapped
